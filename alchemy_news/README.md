# Integrating the Watson AlchemyAPI News Service with Watson Explorer
...
- An [IBM Bluemix](https://ace.ng.bluemix.net/) account (if using AlchemyAPI through Bluemix)
- The [Application Builder proxy](https://github.com/Watson-Explorer/wex-wdc-integration-samples/tree/master/proxy) up and running if you are using WEX < 11.0.
...
2. Watson Explorer App Builder widgets that use the created endpoints.
  - The news article widget displays a list of top news articles with sentiment for a given keyword search.
...
* If using the service through Bluemix, an application is not required to be created or bound to the Alchemy service in Bluemix to use it.
* All basic API keys (no matter where you obtain them) are rate limited by requests/minute and total requests/day.
...
Follow these steps in Application Builder to create the custom widget and add it to the Book Title entity.

1. Navigate to the Pages & Widgets -> Book Title -> detail page.
2. Create a new Custom widget.
3. Set the ID of the widget to be `alchemy_news_trend_graph`
...
6. Save the widget.
7. Go back to the Book Title > detail page.
8. Drag the `alchemy_news_trend_graph` widget to the top of the right column and save the page configuration.
...
- **Failures will happen** - All distributed systems are inherently unreliable and failures will inevitably occur when attempting to call out to an external web service.  Consider how failures should be handled both in your widgets (degrading behavior for user experience) and your endpoints (not crashing).
