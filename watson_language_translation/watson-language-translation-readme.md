# Using Watson Language Translation Service with Watson Explorer

The [Watson Language Translation Service](https://www.ibm.com/smarterplanet/us/en/ibmwatson/developercloud/language-translation.html) enables text to be translated from one language to another.

In the context of Watson Explorer, language translation might prove very useful when combined with a 360 degree view of a customer or other (human) entities. Watson Explorer Application Builder already allows you to view relevant information from disparate data sources from a single web page. Adding translation generated by Watson Language Translation extends the reach of the data you already have.

The goal of this example is to demonstrate how to get started with an integration between Watson Explorer and the Watson Language Translation Service available on IBM Bluemix. By the end of the example you will have added a widget to Watson Explorer Application Builder that sends a query to a federated search engine that returns results in Spanish and then leverages the Watson Language Translation Service to translate those results into English.



## Prerequisites

Please see the [Introduction](/IBM-Watson/wex-appbuilder-samples/blob/master/intro-to-bluemix-for-app-builder.md) for an overview of the integration architecture, and the tools and libraries that need to be installed to create Java-based applications in Bluemix.
...
1. A basic Bluemix application exposing the Watson Language Translation Service as a web service.
...
Once you are signed in, you will need to create the Watson Language Translation service that the example application will be binding to.  In this example, we are calling the service `wex-mt` (this name is already set in the `manifest.yml`).  Since services might be used by multiple applications, this name isn't ideal, but it's perfectly suitable for this example.

```
$> cf create-service language_translation standard wex-lt
```

Build the application web service using [Apache Maven](http://maven.apache.org/). Before performing this step, verify that you are in the BlueMix directory. This will generate a packaged Java WAR called `wex-wdc-language-translation-sample.war`.
...
```
$> cf push -p target/wex-wdc-language-translation-sample.war --random-route
```
...
For this example, the purpose of this entity is to generate English text.  We use a federated search entity to demonstrate translation of "live" text.  That is, Application Builder, and therefore the Watson Language Translation service, has access to this text only at query-time.  The translation could not have been performed prior to entry of the end-user's query.
...
#### Building a Widget that Sends Text to the Watson Language Translation Service

The purpose of this widget is to send all Spanish News result text to the Watson Language Translation service for translation. The results of that translation are then displayed in the Application Builder UI.
...
5. Copy and paste the [code for this widget](ApplicationBuilder/Language Translation.erb) into the Type-specific Configuration
...
![Screen shot of "Watson Language Translation" widget.](ApplicationBuilder/watson-language-translation-widget.png)

__*The completed Watson Language Translation Widget*__

[The widget](ApplicationBuilder/language-translation-widget.erb) is fully commented if you are curious about how the code works or are interested in extending the example functionality in a new widget.
...
- _Maintainability_ - For the example, only the Watson Language Translation Service is built into the Bluemix application. If this were a real application you should consider creating a single Bluemix application for all cloud based cognitive (or other) services used within Bluemix.
...
- _User Experience_ - The example widgets are only meant to demonstrate basic interaction. For a custom application using Application Builder you should carefully consider widget placement, overall look and feel, user needs, and how Watson Language Translation can provide value to end users.
...
## Possible Use Cases for a Watson Language Translation/Watson Explorer Integration

 Here are some ideas to help get you started about how Watson Language translation might be fully integrated into a Watson Explorer application.
